...
def attribute_change_check(request,workout):
    ...
    weight_change = abs(round((current_weight - related_weight) / current_weight,2))*100
    bmi_change = abs(round((current_bmi - related_bmi) / current_bmi,2))*100
    height_change = abs(round((current_height - related_height) / current_height,2))*100
