...
def workout_main(request,pk):
    ...
    context = {'workout':workout, 'excercises' : excercises,'record_list':record_list,'abs_pr_list':abs_pr_list,'page':page}
...
def workout_detail(request,pk):
    ...
    weight_change, weight_change_bool, height_change, height_change_bool, bmi_change, bmi_change_bool = attribute_change_check(request,workout)
...
def profile(request,pk):
    ...
    if request.method == 'POST':
        ...
        given_age = profile.birthday.year if profile.birthday!=None else int(request.POST.get('birthday').split('-')[0])
